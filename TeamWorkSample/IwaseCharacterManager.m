...
#import "IwaseCharacterImageView.h"
#import "ViewController.h"
#import "ScoreManager.h"

@implementation IwaseCharacterManager

static IwaseCharacterManager *sharedData_ = nil;

+ (IwaseCharacterManager *)sharedManager{
    @synchronized(self){
        if (!sharedData_) {
            sharedData_ = [[IwaseCharacterManager alloc] init];
        }
    }
    return sharedData_;
}
...
- (id)init
{
    ...
    if (self) {
        //発生地点
        self.gPoint = CGPointMake(ScreenW*0.25, ScreenH*0.75);
        //スコアに登録
        self.charaImage = [UIImage imageNamed:@"pig1.png"];
...
-(void)doAction{
    ...
    if(generationCount > generationInterval){
        IwaseCharacterImageView *aCIV = [IwaseCharacterImageView new];
        aCIV.image = self.charaImage;
        aCIV.frame = CGRectMake(0, 0, ScreenW/10.0, ScreenW/10.0);
    ...
    //移動
    for(IwaseCharacterImageView *aCIV in self.charaArray){
