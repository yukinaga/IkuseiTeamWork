...
#import "ShimazuCharacterIamgeView.h"
#import "ViewController.h"
#import "ScoreManager.h"

@implementation ShimazuCharacterManager

static ShimazuCharacterManager *sharedData_ = nil;

+ (ShimazuCharacterManager *)sharedManager{
    @synchronized(self){
        if (!sharedData_) {
            sharedData_ = [[ShimazuCharacterManager alloc] init];
        }
    }
    return sharedData_;
}
...
- (id)init
{
    ...
    if (self) {
        //発生地点
        self.gPoint = CGPointMake(ScreenW*0.99999, ScreenH*0.5);
        //スコアに登録
        self.charaImage = [UIImage imageNamed:@"shimazu.png"];
...
-(void)doAction{
    ...
    if(generationCount > generationInterval){
        ShimazuCharacterIamgeView *aCIV = [ShimazuCharacterIamgeView new];
    ...
    //移動
    for(ShimazuCharacterIamgeView *aCIV in self.charaArray){
        ...
        }else if(aCIV.count == 50){
            aCIV.speed = CGVectorMake(0, 0);
        }else if(aCIV.count >= 1000){
            aCIV.count = 0;
            continue;
        }
        aCIV.count++;
